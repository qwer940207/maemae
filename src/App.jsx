...
const storage = {
  get: async () => {
    try {
      // raw fetch 사용 — 대용량 응답도 안정적으로 처리
      const r = await fetch(`${SB_URL}/rest/v1/maemae?id=eq.${DB_ID}&select=data`, { headers: SB_HDR });
      const rows = await r.json();
      return rows[0]?.data ?? null;
    } catch { return null; }
  },
  set: async (value) => {
    const body = JSON.stringify({ id: DB_ID, data: value });
...
const MEDIUM_TAGS = {
  "종배": ["상따", "양봉종배", "음봉종배", "기타"],
  "시초매매": ["상한가 하락시작", "시초 음봉", "신규주", "불타기"],
  "장중매매": ["지지저항", "돌파매매", "투매잡이", "음봉매매", "기타"],
  "스윙": [],
};
const SMALL_TAGS = {
...
const inp = { background: T.input, border: `1px solid ${T.inputBd}`, borderRadius: 8, padding: "10px 12px", color: T.text, fontSize: 15, fontWeight: 400, lineHeight: 1.9, outline: "none", width: "100%", boxSizing: "border-box", fontFamily: "'Noto Sans KR', sans-serif" };
...
export default function App() {
  ...
  // ──────────── MAIN ────────────
  return (
    <div style={{ minHeight: "100vh", background: T.bg, color: T.text, fontFamily: "'Noto Sans KR', sans-serif", fontSize: 15, lineHeight: 1.9, fontWeight: 400 }}>
      {renderCalendar()}
      {renderDashCalendar()}
      {renderAttendance()}
      {renderTrash()}
